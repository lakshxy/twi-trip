...
export default function SwipeCard({ profile, onSwipe, onClick }: SwipeCardProps) {
  ...
  return (
    <Card
      className="swipe-card absolute inset-0 overflow-hidden cursor-pointer modern-card border-0 shadow-2xl bg-white/90"
      onClick={onClick}
    >
      <div className="relative h-80">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-travel-navy/60 via-transparent to-transparent" />
        <div className="absolute bottom-4 left-4 right-4 text-white">
          <div className="flex items-center justify-between mb-2">
            ...
            {profile.rating && Number(profile.rating) > 0 && (
              <div className="flex items-center bg-white/20 backdrop-blur-sm rounded-full px-3 py-1">
                <span className="text-travel-mint text-lg">⭐</span>
          ...
          {(profile.city || profile.state) && (
            <div className="flex items-center text-white/90 mb-2">
              <MapPin className="w-4 h-4 mr-2 text-travel-mint" />
